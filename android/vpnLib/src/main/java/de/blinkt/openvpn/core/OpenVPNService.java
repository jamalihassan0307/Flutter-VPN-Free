...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    private static final int CONNECT_TIMEOUT = 120; // 2 minutes in seconds
    private static final int CONNECT_STATUS_POLL_INTERVAL = 1000; // 1 second in milliseconds
    ...
    public void updateState(String state, String logmessage, int resid, ConnectionStatus level, Intent intent) {
        ...
        if (mManagement != null && mManagement instanceof OpenVpnManagementThread) {
            ((OpenVpnManagementThread) mManagement).setConnectedState(level == ConnectionStatus.LEVEL_CONNECTED);
        }
    ...
    private void handleConnectionTimeout() {
        new Thread(() -> {
            ...
            while (timeoutCounter < CONNECT_TIMEOUT) {
                if (mManagement != null && mManagement.isConnected()) {
                    updateConnectionState(true);
                    return;
                }
                try {
                    Thread.sleep(CONNECT_STATUS_POLL_INTERVAL);
                    timeoutCounter++;
                } catch (InterruptedException e) {
                    return;
                }
            ...
            if (mManagement != null && !mManagement.isConnected()) {
                try {
                    ...
                    VpnStatus.logException("Remote exception during timeout handling", e);
                }
                updateConnectionState(false);
                VpnStatus.logError("Connection timed out after " + CONNECT_TIMEOUT + " seconds");
            }
        }).start();
    }
